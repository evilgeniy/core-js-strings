...
function sumOfCodes(str) {
  if (str) {
    ...
    const totalSum = letterArr.reduce((sum, letter) => {
...
function isPalindrome(str) {
  ...
  let reversestr = normalstr.split('');
...
function extractNameFromTemplate(value) {
  const startIndx = value.indexOf('Hello, ') + 'Hello, '.length;
  const endIndx = value.lastIndexOf('!');
  const name = value.substring(startIndx, endIndx).trim();
  return name;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < 26; i += 1) {
    ...
    data[upper] = String.fromCharCode(((i + 13) % 26) + 65);
    data[lower] = String.fromCharCode(((i + 13) % 26) + 97);
  }

  return str.replace(/[a-zA-Z]/g, (char) => data[char]);
}
